...
import { Question } from "../views/SurveyView";

type Props = {
    questions: Question[],
    onQuestionsUpdate: (question : Question[]) => void
}

export default function SurveyQuestions({questions, onQuestionsUpdate} : Props) {
  const [myQuestions, setMyQuestions] = useState([...questions]);

  const addQuestion = (index : number | undefined) : void => {
  ...
  const questionChange = (question : Question) : Question | void => {
    if (!question) return;
