export default function PublicQuestionView({
  ...
  index,
  answerChanged,
} : {question: any, index: any, answerChanged: any}) {
  let selectedOptions : any = []

  function onCheckboxChange(option : any, $event : any) {
    if ($event.target.checked) {
      selectedOptions.push(option.text)
    } else {
      selectedOptions = selectedOptions.filter((op : any) => op != option.text)
    }
    ...
    <>
      <fieldset className="mb-4">
        ...
        <div className="mt-3">
          ...
          {question.type === "radio" && (
            <div>
              {question.data.options.map((option : any, ind : any) => (
